...
for top10_item in top_10_data:
    top20_item = next((item for item in top_20_data if item['id'] == top10_item['id']), None)
    if top20_item and top10_item['true_answer'] == top10_item['predicted_answer'] and top20_item['true_answer'] != top20_item['predicted_answer']:
...
print(f"{len(incorrect_in_top20)}개의 항목이 top-10에서는 정답이지만, top-20에서는 오답으로 나타났습니다.")
...
print(f"{len(correct_in_top20)}개의 항목이 top-10에서는 오답이지만, top-20에서는 정답으로 나타났습니다.")