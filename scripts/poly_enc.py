...
from encoder_baseline import PCModel
...
class PolyEncoder(PCModel):
    ...
    def dot_production_attention(self, query, key, value, mask=None):
        r"""
        ...
        matmul_qk = torch.bmm(query, key.permute(0, 2, 1)) / scale
        if mask is not None:
            matmul_qk += mask * -1e9
    ...
    def forward(self, query, cand):
        ...
        y_ctxt_i = self.dot_product_attention(
            self.codes.repeat(batch_size, 1, 1), h, h, mask=None
        )

        # Context-Candidate Attention
        # (batch_size, embd_size)
        y_ctxt = self.dot_product_attention(
            y_cand.unsqueeze(1), y_ctxt_i, y_ctxt_i, mask=None
        )
