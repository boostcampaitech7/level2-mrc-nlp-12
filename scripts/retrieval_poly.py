...
def train_polyencoder(
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(train_dataloader[:100], desc=f"Training 에폭: {epoch + 1}"):
            ...
            negative_contexts_input = contexts_input["input_ids"].roll(
                shifts=1, dims=0
            )  # 샘플로 롤링

            # Poly Encoder 모델을 통해 임베딩 생성
            question_embeddings = poly_encoder.cand_encoder(
                questions_input["input_ids"]
            )[:, 0, :]
            context_embeddings = poly_encoder.context_encoder(
                contexts_input["input_ids"]
            )[:, 0, :]
            negative_embeddings = poly_encoder.context_encoder(negative_contexts_input)[
                :, 0, :
            ]
            ...
            positive_similarity = torch.sum(
                question_embeddings * context_embeddings, dim=-1, keepdim=True
            )

            # Negative 유사도 계산 (Dot Product)
            negative_similarity = torch.sum(
                question_embeddings.unsqueeze(1) * negative_embeddings, dim=-1
            )
            ...
            target = torch.cat(
                [
                    ...
                    torch.zeros(
                        batch_size * num_negatives, 1, device=device
                    ),  # Negative -> 0
                ]
            )
...
if __name__ == "__main__":
    ...
    poly_encoder = PolyEncoder(
        args, model_name="klue/bert-base", pooling_method="first", n_codes=64
    )
    ...
    if os.path.exists(embedding_file):
        ...
        for file_name in tqdm(
            sorted(os.listdir(embedding_dir)), desc="임베딩 파일 불러오는 중..."
        ):
            if file_name.startswith("context_embeddings_") and file_name.endswith(
                ".pt"
            ):
            ...
            for text in batch_text:
                ...
                if batch_count % save_interval == 0:
                    partial_file = os.path.join(
                        embedding_dir, f"context_embeddings_{batch_count}.pt"
                    )
    ...
    question_tokens = tokenizer(
        question, return_tensors="pt", truncation=True, padding=True, max_length=512
    ).input_ids.to(device)
    with torch.no_grad():
